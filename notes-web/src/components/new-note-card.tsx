...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    //Verificar se a API está disponivel no navegador
    const isSpeechRecognitionAPIAvailable =
      "SpeechRecognition" in window || "webkitSpeechRecognition" in window;

    if (!isSpeechRecognitionAPIAvailable) {
      alert("Infelizmente seu navegador não suporta a aplicação de gravação");
      return;
    }
    ...
    speechRecognition.continuous = true; // Fica gravando até pedir para parar de gravar
    //Caso essa informação fique false, caso o usuário pare de falar, automaticamente a API irá parar a gravação

    speechRecognition.maxAlternatives = 1; // Trazer apenas uma alternativa para uma que a apliação não tenha entendido direito
    speechRecognition.interimResults = true; // Va trazendo os resultados enquanto o usuário está falando, e não só quando terminar de falar
    ...
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 p-5 text-left gap-3 hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400 outline-none">
        ...
        <p className="text-sm leading-6 text-slate-400">
          Grave uma nota em audio que será convertida em texto
        </p>
      </Dialog.Trigger>
